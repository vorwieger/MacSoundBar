...
@property (nonatomic, strong) NSValue * hotKeyRef;
...
- (void) invokeWithEvent:(NSEvent *)event {
	...
	if (target != nil && action != nil && [target respondsToSelector:action]) {
        ...
        void (*func)(id, SEL, NSEvent*, id) = (void *)imp;
        func(target, action, event, object);
	}
#if NS_BLOCKS_AVAILABLE
	else if (task != nil) {
		task(event);
...
- (void) unregisterAllHotKeys {
	[self unregisterHotKeysMatchingPredicate:[NSPredicate predicateWithFormat:@"hotKeyRef != NULL"]];
}
